...
## 8. PR分割ルール
- PRは機能単位（Week / Closet / Suggestion / Settings）で分割する
- 複数機能を同時変更する場合は、機能ごとに別PRを作成する
- 共通基盤変更は可能な限り独立PRに切り出す


## 9. 実装チェックリスト（進行管理用）

> 運用ルール: 実装前に本チェックリストを確認し、完了時にチェックを更新する。  
> 迷った場合は本ドキュメントを再確認し、それでも不明なら相談する。

### フェーズA（画面骨組み）
- [x] Week: 週詳細画面・日別編集画面を独立View化
- [x] Week: AI重み設定画面（最低限UI）を追加
- [x] Week: 日別編集ViewModel（再生成/保存）を追加
- [x] Suggestion: 条件入力・結果・詳細の分離
- [x] Suggestion: 条件入力→詳細のPush導線
- [x] Suggestion: ViewModel連携による提案生成ステータス表示
- [x] Settings: 地域設定フォーム骨組み化（Picker + 現在値表示）
- [x] Settings: アカウント項目をインライン「準備中」で表示
- [x] Settings: ViewModel連携で地域/通知/保存ステータスを管理
- [x] Closet: 一覧と編集フォームを分離し、追加/編集のPush導線を実装
- [x] Closet: ViewModel連携で一覧をRepositoryデータ表示
- [x] Week: 生成→日別編集→保存→週詳細で反映のデモ導線を実装

### フェーズB（永続化）
- [ ] SwiftDataモデルの作成（WeekPlan中心）
- [ ] `WeekPlanRepository` のSwiftData実装
- [ ] 起動時ロード / 保存時更新UseCaseの整備
- [ ] 永続化層の単体テスト追加

### フェーズC（外部連携準備）
- [ ] WeatherRepositoryのHTTP実装方針決定
- [ ] SuggestionRepositoryのHTTP実装方針決定
- [ ] API失敗時のリトライ/エラー表示ポリシー定義

### 品質ゲート
- [x] `swift test` がローカルで成功
- [x] `ci.yml` による自動テスト
- [x] `pr-scope-check.yml` による機能単位PRチェック
