...
### フェーズB（永続化）
- [x] SwiftDataモデルの作成（WeekPlan中心）
- [x] `WeekPlanRepository` のSwiftData実装
- [x] 起動時ロード / 保存時更新UseCaseの整備
- [x] 永続化層の単体テスト追加

### フェーズC（外部連携準備）
- [x] WeatherRepositoryのHTTP実装方針決定
- [x] SuggestionRepositoryのHTTP実装方針決定
- [x] API失敗時のリトライ/エラー表示ポリシー定義
